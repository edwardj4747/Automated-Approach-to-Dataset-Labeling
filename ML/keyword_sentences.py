...
def get_text(paper, from_manually_reviewed=False):
    if from_manually_reviewed:
        preprocessed_location = '../convert_using_cermzones/manually_extracted_text/'
    else:
        preprocessed_location = '../convert_using_cermzones/preprocessed/'
...
def remove_all_non_keywords(sentence):
    ...
    sentence = substitute_keywords(sentence)
...
def substitute_keywords(sentence):
...
with open('ml_data/ground_truths.json') as f:
    ground_truths = json.load(f)

all_paper_list = list(raw_data.keys())
...
for paper in all_paper_list:
    ...
    for sent in text.split("."):
        just_keywords, number_keywords = remove_all_non_keywords(sent)
        # if paper == 'LNW9PJEF':
        #     print(just_keywords)
    ...
    def is_good_possibility(possibility):
        ...
        if value > 0 and not in_potential_sequence:
            ...
            if value == 0:
                ...
                if cps_num_zeros > max_sent_distance:
                    ...
                    if is_good_possibility(possibilities):
...
# with open('ml_data/KEYWORD_sentences.json', 'w', encoding='utf-8') as f:
