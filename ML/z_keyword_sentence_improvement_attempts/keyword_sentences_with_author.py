...
'''
    This started by directly copying and pasting keyword sentences.
    New changes involve adding in the author as keywords
'''
...
def remove_all_non_keywords(sentence, keywords):
    ...
    authors_list = list(keywords['author_names'])
    author_last_names = list([a.split(' ')[-1] for a in authors_list])
    ...
    for word in sentence.split(" "):
        ...
        if word != 'no' and (
                word in missions_short or word in instruments_short or word in variables_short or word in author_last_names
                or re.fullmatch(r'v\d\.?\d?(/v\d)*', word) or re.fullmatch(r'L\d', word)):
            new_sentence += word + " "
        elif word.endswith("."):
            new_sentence += ". "
...
def substitute_keywords(sentence, keywords):
    ...
    authors = list(keywords['author_names'])
    ...
    # Versions
    # lowercase_sentence = re.sub(r'', '(v\\1 v\\2)', lowercase_sentence)  # v3/3 -> (v3 v3)
    # see if dataset versions are mentioned and replace 'version 4' with v4
    lowercase_sentence = re.sub(r'[vV]ersion ([0-9])(\.\d)?/?([vV]ersion)?(\d)?(\.?\d?)?', 'v\\1/v\\4', lowercase_sentence)
    ...
    for author in authors:
        ...
        lowercase_sentence = re.sub(rf'{author}', f'{author_last_name}', lowercase_sentence)
...
def run_keyword_sentences(save=False, alt_path=''):
    ...
    paper = '2AQ88HE3.txt'
#
#     s = "the author livesey used aura mls version 2.3 data"
#     s = 'In this study, cos temperature data from a satellite, Aura Microwave Limb Sounder (MLS) Version 3.3/3.4 Level 2 \
# [Waters et al., 2006], are also used to examine the dependence of the PMSE intensity on the\
# background temperature cos'
#     print(remove_all_non_keywords(s, keywords))
#     exit()
    ...
    max_num_sentences = 4
    total_required = 6
    param_string = f'd{max_sent_distance}_mn{max_num_sentences}_tr{total_required}'
    ...
    if save:
        with open('data/' +  str(num_papers_in_dictionary) + 'author_keywords_attempt_' + param_string + '.json', 'w', encoding='utf-8') as f:
...
if __name__ == '__main__':
    run_keyword_sentences(alt_path='../', save=True)